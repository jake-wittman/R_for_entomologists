...
```{r install_packages, echo = TRUE, message = FALSE, warning = FALSE}
...
You can read this R code as "store 4 in x" or "assign the number 4 to the letter x". This is a basic application of the assignment operator. Let's look at a slightly more complicated application. Make sure to read the comments along with the code.
...
*Side note*: The pipe operator `%>%` can be kind of frustrating to type. Thankfully there's a keyboard shortcut for it in R Studio! If you're using a Windows machine use **ctrl + shift + m** to insert the pipe operator. While we're at it, there's a shortcut for the assignment operator `<-` too! Again, on a Windows machine type **alt + -**. (Sorry, I don't know the Mac OS shortcuts. I'd guess that you replace **Ctrl** with **Cmd**. A google search might help?)

## Chaining multiple functions together with the pipe

You can use the pipe operator to link multiple functions together, which is often useful when summarizing data. The code for this brief example is below the following paragraph. I'll briefly explain some of the code before we get there but we'll cover them in more depth in a bit. In this example, I'll show you three methods of achieving the same result to highlight again why pipes are useful or preferred over other methods. The first method will nest multiple functions to avoid saving unneeded intermediate objects while still not using pipes.In the second method, we'll save every intermediate step as an object on our way to our final goal without using any pipes.  The final method will use pipes.
...
```{r intermediate}
# Make a new data frame called setosa_iris which holds only the observations where
# species is "setosa"
...
Based on the pairwise comparisons of our three data frames, all three methods produced the same object. So which is the best? If you guessed that Id say the pipe method is the best and you'd be right. Let's start by comparing the piped method to our nested function. 
...
Now, let's compare the piped method to our intemediate steps method. Look at the functions filter, select, and arrange in each of the two examples we're focusing on. You might notice that in the intermediate steps example, each of those three functions has two arguments: the first is the data frame being used and the second specifies the part of the data frame on which the action should be performed. Why do the functions in the piped example not need to specify what data is being used? These functions (and most functions in the `tidyverse`) are written with the pipe operator in mind. When you use tidyverse functions in a pipe, they default to using output piped from another function as data in the next function. If you wanted to be explicit, you can specify the data being used as you pipe from one function to the next this way:

```{r}
# The "." stands in for the object being piped into the function
piped_sepal_length <- iris %>% 
   filter(., Species == "setosa") %>% 
   select(., Sepal.Length) %>% 
   arrange(., Sepal.Length)
...
In this code, the "." is explicitly standing in for the data object, which is being piped into the function. We piped the iris dataset into the `filter()` function and the "." represents what is being piped in. Similarly, we filter the iris dataset and the pipe that filtered dataset into the `select()` function. We specify the data to be selected explicitly by including the ".", which represents the piped in input of the filtered  datset. Using the "." allows us to work with functions that wouldn't otherwise play well with pipes (as in, they always require that the data argument be specified).
Not every function you can use in R will play well with piped output. For example:

```{r, eval = FALSE}
# This won't work - the mean() function is not part of the tidyverse so it doesn't
# inherently play well with the pipe
...
In this example, we wanted to run a simple linear regression to look at the relationship between sepal width and sepal length. The first line won't work - the `lm()` function was not written with the pipe in mind (it's not part of the tidyverse) and so it doesn't know what to do with `iris` when `iris` gets piped to `lm()`. We can tell `lm()` what to do with `iris` by using the period and setting `data = .`. This tells `lm()` that the piped in object `iris`, represented by the period, should be used as the data argument for the `lm()` function.

Pipes are also useful because you don't end up with a lot of intermediate objects that you might not use again. Pipes keep your programming environment clean by eliminating many one-use objects which in turn makes it easier to remember what objects you need for specific tasks. In the intermediate example, we had to make several objects that we may not use again at any point. We can keep our environment clean by not saving them in the first place.
